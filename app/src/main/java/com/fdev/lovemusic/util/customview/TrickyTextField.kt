...
class TrickyTextField(
    ...
    private val editText: TextInputEditText = TextInputEditText(context, attributeSet)
        ...
        override fun afterTextChanged(s: Editable?) {
            when(val errorData = isError(s.toString())){
                is ErrorData.NotError -> {
                    println("tipe not error " + errorData.isError)
                    onSuccess()
                }
                is ErrorData.Error -> {
                    println("tipe error " + errorData.isError)
        ...
        context.theme.obtainStyledAttributes(
            ...
            try {
                ...
                editText.apply {
                    ...
                    textSize = pixelsToSp(getDimension(R.styleable.TrickyTextField_textSize, 0f))
